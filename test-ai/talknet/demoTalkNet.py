...
if LOCAL_MACHINE:
	...
	pretrained_model_path = ".cache/models/pretrain_TalkSet.model"

	
# 全域變數，但 save_path 會在 main 函數中被覆蓋
...
if os.path.isfile(pretrained_model_path) == False: # Download the pretrained model
    ...
    cmd = "python3.11 -m gdown --id %s -O %s"%(Link, pretrained_model_path)
    subprocess.call(cmd, shell=True, stdout=None)

def scene_detect(video_path, save = False, start_frame = 0, end_frame = None, pyworkPath = None):
...
def predict_faces(DET, start_frame = 0, pyframesPath = None, pyworkPath = None):
	# GPU: Face detection, output is the list contains the face location and score in this frame
	if pyframesPath is None:
		raise ValueError("pyframesPath is required")
...
def predict_faces_from_frames(DET, frames, start_frame = 0, pyworkPath = None):
	# GPU: Face detection, output is the list contains the face location and score in this frame
	dets = []
	import time
...
def crop_video(track, cropFile, start_frame = 0, pyframesPath = None, audioFilePath = None):
...
def crop_video_from_frames(track, cropFile, frames, start_frame = 0, audioFilePath = None):
	# CPU: crop the face clips
	if audioFilePath is None:
		raise ValueError("audioFilePath is required")
	vOut = cv2.VideoWriter(cropFile + 't.avi', cv2.VideoWriter_fourcc(*'XVID'), 25, (224,224))# Write video
	dets = {'x':[], 'y':[], 's':[]}
	for det in track['bbox']: # Read the tracks
		dets['s'].append(max((det[3]-det[1]), (det[2]-det[0]))/2) 
...
def evaluate_network(s, files, pycropPath = None):
	# GPU: active speaker detection by pretrained TalkNet
	if pycropPath is None:
		raise ValueError("pycropPath is required")
	allScores = []
...
def visualization(tracks, scores, pyframesPath = None, pyaviPath = None):
	# CPU: visulize the result for video format
	if pyframesPath is None or pyaviPath is None:
		raise ValueError("pyframesPath and pyaviPath are required")
	flist = glob.glob(os.path.join(pyframesPath, '*.jpg'))
	flist.sort()
...
def visualization_from_frames(tracks, scores, frames, pyaviPath = None):
	if pyaviPath is None:
		raise ValueError("pyaviPath is required")
...
def main(
	...
	print(f"main() 使用的 save_path: {save_path}")
	...
	t = time.time()
	scene = scene_detect(video_path, save = True, start_frame = start_frame, end_frame = int(end_seconds * fps), pyworkPath = pyworkPath)
	...
	if face_boxes:
		...
		if (end_seconds - start_seconds) * fps < STORE_FRAMES_IN_MEMORY_THRESHOLD:
			faces = predict_faces_from_frames(DET, frames, pyworkPath = pyworkPath)
		else:
			faces = predict_faces(DET, pyframesPath = pyframesPath, pyworkPath = pyworkPath)
	...
	for ii, track in enumerate(allTracks):
		if (end_seconds - start_seconds) * fps < STORE_FRAMES_IN_MEMORY_THRESHOLD:
			vidTracks.append(crop_video_from_frames(track, os.path.join(pycropPath, '%05d'%ii), frames, audioFilePath = audioFilePath))
		else:
			vidTracks.append(crop_video(track, os.path.join(pycropPath, '%05d'%ii), pyframesPath = pyframesPath, audioFilePath = audioFilePath))
	...
	files.sort()
	scores = evaluate_network(s, files, pycropPath = pycropPath)
	...
	if return_visualization:
		...
		if (end_seconds - start_seconds) * fps < STORE_FRAMES_IN_MEMORY_THRESHOLD:
			visualization_from_frames(vidTracks, scores, frames, pyaviPath = pyaviPath)
		else:
			visualization(vidTracks, scores, pyframesPath = pyframesPath, pyaviPath = pyaviPath)
