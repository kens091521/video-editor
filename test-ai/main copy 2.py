...
def cut_video(input_path, output_path, start, end):
    # 使用 ffmpeg 擷取區間並重新編碼，避免 h264 mmco 錯誤
    cmd = [
        "ffmpeg",
        "-y",
        "-i", input_path,
...
def analyze_video(video_path):
    ...
    from mixModel import getOutcomeJson
    return getOutcomeJson(video_path)
...
def main():
    if CONFIG["packing"]:
        # 取得 output 目錄下所有資料夾，並依數字排序
        output_root = CONFIG["output_dir"]
        subfolders = [d for d in os.listdir(output_root) if os.path.isdir(os.path.join(output_root, d))]
        # 過濾純數字資料夾並排序
        subfolders_sorted = sorted(subfolders, key=lambda x: int(x) if x.isdigit() else float('inf'))
